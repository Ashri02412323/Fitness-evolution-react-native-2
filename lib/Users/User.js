...
const setNewPass = async (email, password) => {
  ...
  const checkIfLoggedIn = async (setUser) => {
    try {
      ...
      console.log("Token in checkIfLoggedIn: ", token);
      
      if (token) {
        const user = await getUser(token);
        setUser(user);
        console.log("User in checkIfLoggedIn: ", user);
        return true;
      }else {
        return false;
      }
...
export { LoginUser, SignUpUser, VerifyEmail, validateEmail, validatePassword,validateFullName, setNewPass, getToken, logoutUser, checkIfLoggedIn };