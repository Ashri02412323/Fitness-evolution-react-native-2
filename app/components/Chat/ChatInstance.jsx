...
const ChatInstance = ({ user, profile, role, userName }) => {
  ...
  const prevReceivedRef = useRef(received);
  const prevLastMessagesRef = useRef(lastMessages);
  ...
  useEffect(() => {
    // if(prevLastMessagesRef.current !== lastMessages){
    if(!chats) return;
    let userChats = chats[user?.id];
    if (userChats && userChats.length > 0) {
      let lastMsg = userChats[userChats.length - 1];
  ...
  }, [lastMessages, user?.id,chats]);

  useEffect(() => {
    if(prevReceivedRef.current !== received){
      let count = 0;
  ...
  return (
    <Pressable onPress={() => handleNavigate(userName, user)} className="flex flex-row items-center justify-between py-4 px-2 mb-1 relative">
